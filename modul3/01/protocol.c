...
void send_msg(int fd, const void *msg, size_t size) {
    ...
    while (total < size) {
        ...
        if (w <= 0) { perror("send_msg write error"); exit(1); }
        total += w;
    }
}
...
void recv_msg(int fd, void *msg, size_t size) {
    ...
    while (total < size) {
        ...
        if (r <= 0) { perror("recv_msg read error"); exit(1); }
        total += r;
    }
}
...
void recv_file_data(int read_fd, int file_fd, off_t size) {
    ...
    while (remaining > 0) {
        ...
        if (r <= 0) { perror("recv_file_data read error"); break; }
        
        ssize_t w = write(file_fd, buffer, r);
        if (w <= 0) { perror("recv_file_data write error"); break; }
